...
diffFrameNum = 0;
for i = 2:size(movieFrames,2)
    diffFrame = movieFrames(2).cdata-fliplr(movieFrames(i).cdata);
    imshow(diffFrame);
%    pause(1);
...
plot(frame0Stats(2).Centroid(1),frame0Stats(2).Centroid(2),'ro');
hold off;
pause (1);
...
projectionImages = [];
for i = 1:diffFrameNum
    projectionImages = [projectionImages; movieFrames(i).cdata(:,:,1)];
end       
...
for i = 1:1:diffFrameNum
    %aligned sinogram
    currentAlignedImage = movieFrames(i).Aligned;
    blueImage = currentAlignedImage(:,:,3);
    sinoAligned = [sinoAligned; blueImage(sliceLocation,:)];
end
...
for i = 1:1:diffFrameNum
    %aligned sinogram
    currentAlignedImage = imtranslate(movieFrames(i).Aligned,[-20 0]);
    blueImage = currentAlignedImage(:,:,3);
    sino = [sino; blueImage(sliceLocation,:)];
end



%take a look at the sinogram
imagesc(sino')
colormap(hot)
colorbar
...
output_size = round(max(size(movieFrames(1).cdata))/2^.5);
recondImage = iradon(sinoAligned', floor(theta), 'spline','Hamming', output_size);
recondImage2 = iradon(sino', floor(theta), 'spline','Hamming', output_size);

figure
montage({recondImage, recondImage2});
...
for i = 1:1:size(imageStack),2)
    currentImage = imtranslate(imageStack(i),(alignment);
    sino = [sino; imageStack(slice,:)];
end
end
